 ...
 * @modified garis.suero@gmail.com 12/28/2012
 */
public class NumberToWord {
    ...
    static public class DefaultProcessor extends AbstractProcessor {
        ...
        static private String POUND = "Pound";
        static private String PENCE = "Pence";
        ...
        public String getName(String value) {
            ...
            boolean integerPlural = false;
            boolean decimalPlural = false;
            if (value.startsWith("-")) {
                negative = true;
            ...
            if (0 <= decimals) {
                ...
                decimals = value.indexOf(".");
                decimalValue = value.substring(decimals + 1);
                value = value.substring(0, decimals);
            }
            if (Integer.valueOf(value.substring(0, decimals)) > 1) {
                integerPlural = true;
            }
            if (Integer.valueOf(decimalValue) > 1) {
                decimalPlural = true;
            }
            ...
            if (!(null == decimalValue || decimalValue.isEmpty())) {
                name = name.concat(SEPARATOR).concat(POUND+(integerPlural ? "s" : "")).concat(SEPARATOR).concat(UNION_AND).concat(SEPARATOR)
                    .concat(processor.getName(decimalValue))
                    .concat(SEPARATOR).concat(PENCE+(decimalPlural ? "s" : ""));
                    //.concat(SCALE.getName(-decimalValue.length()))
            }

            return (!isCapitalize() ? name.toLowerCase() : name);
        }
